...
extern uint32_t __sram_data_start;
extern uint32_t __sram_data_end;
extern uint32_t __srom_data_start;
extern uint32_t __srom_data_end;
extern uint32_t __sram_bss_start;
extern uint32_t __sram_bss_end;
...
void Reset_Handler()
{
	...
	//copy .sram.data to ram
	for (src = &__srom_data_start, dest = &__sram_data_start; dest < &__sram_data_end; src++, dest++)
		*dest = *src;

	//zero .sram.bss
	while (dest < &__sram_bss_end)
		*dest++ = 0;
